...
from .data_definitions import PlanFile, PlanFileMission, SimpleMissionItem, ComplexMissionItem, \
    TransectStyleComplexItem, CameraCalcData
...
class FlightPlanSerializer(serializers.ModelSerializer):
    ...
    def process_simple_item(self, mission_item):
        ...
        simple_mission = SimpleMissionItem(AMSLAltAboveTerrain=amsl_alt, Altitude=alt, AltitudeMode=alt_mode,
                                           AutoContinue=auto_continue, Command=command, DoJumpId=jump_id, Params=params,
                                           Frame=frame)
    ...
    def parse_and_validate_plan_file(self, plan_file_json):
        ...
        for mission_item in mission_items:
            ...
            if (mission_type == "SimpleItem"):
                ...
                try:
                    ...
                    camera_calc_data = CameraCalcData(AdjustedFootprintFrontal=camera_calc['AdjustedFootprintFrontal'],
                                                      AdjustedFootprintSide=camera_calc['AdjustedFootprintSide'],
                                                      CameraName=camera_calc['CameraName'],
                                                      DistanceToSurface=camera_calc['DistanceToSurface'],
                                                      DistanceToSurfaceRelative=camera_calc[
                                                          'DistanceToSurfaceRelative'], version=camera_calc['version'])
                except Exception:
                    camera_calc_data = CameraCalcData(AdjustedFootprintFrontal=0, AdjustedFootprintSide=0, CameraName=0,
                                                      DistanceToSurface=0, DistanceToSurfaceRelative=0, version=0)
                ...
                transect_style_complex_item = TransectStyleComplexItem(CameraCalc=camera_calc_data,
                                                                       CameraShots=camera_shots,
                                                                       CameraTriggerInTurnAround=camera_trigger_in_turn_around,
                                                                       FollowTerrain=follow_terrain,
                                                                       HoverAndCapture=hover_and_capture,
                                                                       Items=all_simple_items,
                                                                       Refly90Degrees=refly_90_degrees,
                ...
                complex_mission = ComplexMissionItem(EntryPoint=entry_point, CorridorWidth=corridor_width,
                                                     TransectStyleComplexItem=transect_style_complex_item, Angle=angle,
                                                     ComplexItemType=complex_item_type, EntryLocation=entry_location,
                                                     FlyAlternateTransects=fly_alternate_transects, Type=c_mission_type,
                                                     Version=version, Polygon=polygon, Polyline=polyline)
        ...
        m = PlanFileMission(CruiseSpeed=mission['cruiseSpeed'], FirmwareType=mission['firmwareType'],
                            HoverSpeed=mission['hoverSpeed'], PlannedHomePosition=mission['plannedHomePosition'],
                            VehicleType=mission['vehicleType'], Version=mission['version'], Items=all_mission_items)

        pf = PlanFile(FileType=plan_file_json['fileType'], Version=plan_file_json['version'],
                      GroundStation=plan_file_json['groundStation'], Mission=m)
        return pf

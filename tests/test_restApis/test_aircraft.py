...
class TestAircraft(TestApiEndpoints):
    fixtures = ['Aircraft', 'Engine', 'Operator', 'Manufacturer', 'TypeCertificate', 'Address', 'Authorization',
                'Activity']
    ...
    def test_aircraft_list_returns_200(self):
        ...
        self.assertEqual(res.json()[0]['id'], self.get_pk_for_modal('Aircraft'))

    def test_aircraft_detail_returns_200(self):
        self.setUpClientCredentials([self.READ_SCOPE, self.WRITE_SCOPE])
        url = reverse('aircraft-detail', kwargs={'pk': self.get_pk_for_modal('Aircraft')})

        res = self.client.get(url)
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertEqual(res.json()['id'], self.get_pk_for_modal('Aircraft'))
